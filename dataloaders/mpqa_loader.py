...
class Mpqa_Dataset(Dataset):
    def __init__(self,tokenizer,split,text_infilling=False,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
        if local_dir:
            ...
            dataset = load_dataset("rahulsikder223/SentEval-MPQA",cache_dir=cache_dir,download_mode="reuse_cache_if_exists")
        else:
        ...
        if text_infilling:
            if task=="mlm":
                ...
                def process_infilling(example):
                    example["sentence"] = example["sentence"]+"It was"
                    example['text_labels'] = Text_labels[example['label']]
                    return example
...
def get_mpqa_loader(batch_size=16,tokenizer=None,text_infilling=True,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
    train_dataset = Mpqa_Dataset(tokenizer,"train",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)
    val_dataset = Mpqa_Dataset(tokenizer,"val",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)

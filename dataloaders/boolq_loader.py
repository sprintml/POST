...
class BoolQ_Dataset(Dataset):
    def __init__(self,tokenizer,split,text_infilling=False,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
        if split=="train":
            dataset =  load_dataset("boolq",split="train").shuffle(seed=seed)
        elif split=="val":
        ...
        if text_infilling:
            if task=="clm":
                def process_infilling(e): 
                    template_str = "Context: {context}\nQuestion: {question}\nA.{c1}\nB.{c2}\nThe answer is A or B? "
                    c1="no"
                    c2="yes"
                    e["sentence"] = template_str.format(context=e["passage"],question=e['question'], c1=c1,c2=c2)
                    e["text_labels"] = "A" if e["answer"]==False else "B"
                    e['label'] = 1 if e['answer'] else 0
                    return e
            elif task=="mlm":
                def process_infilling(e): 
                    template_str = "Context: {context}\nQuestion: {question}\nA.{c1}\nB.{c2}\nThe answer is <mask>"
...
def get_boolq_loader(batch_size=16,tokenizer=None,text_infilling=False,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
    train_dataset = BoolQ_Dataset(tokenizer,"train",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)
    val_dataset = BoolQ_Dataset(tokenizer,"val",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)

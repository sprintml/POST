...
MAX_LENGTH = 256
LABEL_MAP = {"business":0,"sports":1,"politics":2,"health":3,"entertainment":4,"tech":5}


class Arisetv_Dataset(Dataset):
    def __init__(self,tokenizer,split,text_infilling=False,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
        if local_dir:
            ...
            dataset = load_dataset("okite97/news-data",cache_dir=cache_dir,download_mode="reuse_cache_if_exists")
        else:
        ...
        # add label according to Category
        def process_label_and_title(example):
            example["label"] = LABEL_MAP[example["Category"]]
            example['Title'] = example["Title"] if example["Title"] else "None"
            return example
        dataset = dataset.map(process_label_and_title)
        ...
        if text_infilling:
            if task=="mlm":
                def process_infilling(example):
                    example["sentence"] = "Title:"+example["Title"]+"Excerpt:"+example['Excerpt']+"It's about <mask>."
                    example['text_labels'] = example['Category']
                    return example
            elif task=="clm":
                def process_infilling(example):
                    example["sentence"] = "Title:"+example["Title"]+"Excerpt:"+example['Excerpt']+"It's about"
                    example['text_labels'] = example['Category']
                    return example
            dataset = dataset.map(process_infilling)
        else:
            def process(example):
                example["sentence"] = "Title:"+example["Title"]+"Excerpt:"+example['Excerpt']
                example['text_labels'] = example['Category']
                return example
...
def get_arisetv_loader(batch_size=16,tokenizer=None,text_infilling=False,forpruning=False,cache_dir=None,local_dir=None,max_length=MAX_LENGTH,seed=None,task="mlm"):
    train_dataset = Arisetv_Dataset(tokenizer,"train",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)
    val_dataset = Arisetv_Dataset(tokenizer,"val",text_infilling,forpruning,cache_dir,local_dir,max_length=max_length,seed=seed,task=task)
